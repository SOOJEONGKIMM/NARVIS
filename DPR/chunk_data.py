...
class DataChunk:
    ...
    def __init__(self, chunk_size=100):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("BM-K/KoSimCSE-roberta-multitask")
    ...
    def chunk(self, input_file):
        ...
        for art in input_txt:
            ...
            if len(encoded_txt) < 20:  # 본문 길이가 subword 10개 미만인 경우 패스
                logger.debug(f"title {title} has <20 subwords in its article, passing")
                continue
...
if __name__ == "__main__":
    ...
    # app = DataChunk(chunk_size = 100)
    # item = []
    # num = 0
    # for i,path in enumerate(tqdm(glob('text/*/wiki_*'))):
    #     ret = app.chunk(path)
