...
from transformers import AutoTokenizer, AutoModel,BertModel, AutoModelForMaskedLM
import indexers
...
class IndexRunner:
    ...
    def __init__(
        ...
        self.encoder = AutoModel.from_pretrained("/home/seongilpark/fid/my_model_second_q_128_lowlr").to(self.device)
        self.emb_sz = 768
        ...
        self.indexer.init_index(self.emb_sz)
    ...
    def run(self):
        ...
        for batch in tqdm(self.loader, desc="indexing"):
            ...
            with torch.no_grad():
                output, _ = self.encoder(p, p_mask, return_dict=False)
            _to_index += [(cur + i, _emb) for i, _emb in enumerate(output[:,0,:].cpu().numpy())]
            cur += output[-1].size(0)
...
if __name__ == "__main__":
    IndexRunner(
        data_dir="text",
        index_output="2050iter_flat_roberta",
    ).run()
    # test_loader()
